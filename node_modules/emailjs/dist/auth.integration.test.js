...
function send({ authMethods = [], authOptional = false, secure = false, password = 'honey', } = {}) {
    return new Promise((resolve, reject) => {
        ...
        const server = new SMTPServer({
            ...
            onAuth(auth, _session, callback) {
                ...
                if ((method === AUTH_METHODS.XOAUTH2 && pw != null
                    ? accessToken === 'pooh'
                    : username === 'pooh') &&
                    ...
                    return callback(new Error(`invalid user or pass: ${username || accessToken} ${pw}`));
                }
            },
...
describe('Auth Integration', () => {
    ...
    it('no authentication (encrypted) should succeed', async () => {
        await expect(send({ authOptional: true, secure: true })).resolves.toBeUndefined();
    });
    ...
    it('PLAIN authentication (encrypted) should succeed', async () => {
        await expect(send({ authMethods: [AUTH_METHODS.PLAIN], secure: true })).resolves.toBeUndefined();
    });
    ...
    it('LOGIN authentication (encrypted) should succeed', async () => {
        await expect(send({ authMethods: [AUTH_METHODS.LOGIN], secure: true })).resolves.toBeUndefined();
    });
    ...
    it('XOAUTH2 authentication (encrypted) should succeed', async () => {
        await expect(send({ authMethods: [AUTH_METHODS.XOAUTH2], secure: true })).resolves.toBeUndefined();
    });
