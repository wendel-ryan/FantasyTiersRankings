...
describe('SMTPClient', () => {
    ...
    it('client deduplicates recipients', () => {
        ...
        const stack = new SMTPClient({ host: 'localhost' }).createMessageStack(new Message(msg));
    ...
    it('client accepts array recipients', () => {
        ...
        msg.header.to = [msg.header.to];
        msg.header.cc = [msg.header.cc];
        msg.header.bcc = [msg.header.bcc];
    ...
    it('client constructor throws if `password` supplied without `user`', () => {
        expect(() => new SMTPClient({ user: 'anything', password: 'anything' })).not.toThrow();
        expect(() => new SMTPClient({ password: 'anything' })).toThrow();
        expect(() => new SMTPClient({
            username: 'anything',
            password: 'anything',
        })).toThrow();
    });
    it('sendAsync rejects if message is invalid', async () => {
        ...
        await expect(client.sendAsync({})).rejects.toThrow('message is not a valid Message instance');
    });
    it('client callbacks with error if no recipients found', async () => {
        ...
        await expect(client.sendAsync(msg)).rejects.toThrow('No recipients found in message');
    });
    ...
    it('client callbacks with error if message fails validity check', async () => {
        ...
        await expect(client.sendAsync(msg)).rejects.toThrow('Message must have a `from` header');
    });
    it('client handles stream error during sending', async () => {
        const client = new SMTPClient({ host: 'localhost' });
        let currentState = SMTPState.NOTCONNECTED;
    ...
    it('throws if stack.to is invalid in _sendrcpt', () => {
        ...
        expect(() => client._sendrcpt({ to: null })).toThrow('stack.to must be array');
    });
    ...
    it('client handles login error', async () => {
        ...
        const mockSmtp = {
            connect: (cb) => cb(),
            authorized: () => false,
            login: (cb) => cb(new Error('Login failed')),
    ...
    it('sendAsync rejects if _canMakeMessage fails', async () => {
        const client = new SMTPClient({ host: 'localhost' });
        await expect(client.sendAsync({ from: 'me', text: 'hi' })).rejects.toThrow('message is not a valid Message instance');
        await expect(client.sendAsync({ from: 'me', to: 'you' })).rejects.toThrow('message is not a valid Message instance');
    });
