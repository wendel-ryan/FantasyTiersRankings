...
const tarFixtureUrl = new URL(`file://${resolve(FIXTURES_DIR, 'postfix-2.8.7.tar.gz')}`);
...
const server = new SMTPServer({
    ...
    async onData(stream, _session, callback) {
        try {
            ...
            parseMap.set(mail.subject, mail);
            callback();
        }
        catch (err) {
            callback(err);
        ...
        try {
            client.send(new Message(headers), (err) => {
                if (err) {
                    ...
                    const checkMap = () => {
                        const mail = parseMap.get(headers.subject);
                        if (mail) {
                            resolve(mail);
                        }
                        else {
...
describe('Message Integration', () => {
    ...
    it('html with image embed message', async () => {
        const htmlFixture2Url = new URL(`file://${resolve(FIXTURES_DIR, 'smtp2.html')}`);
        const imageFixtureUrl = new URL(`file://${resolve(FIXTURES_DIR, 'smtp.gif')}`);
        const msg = {
        ...
        const mail = await send(msg);
        if (mail.attachments) {
            expect(mail.attachments[0].content.toString('base64')).toBe(readFileSync(imageFixtureUrl, 'base64'));
        }
        else {
            throw new Error('Attachments missing');
        }
        expect(mail.html).toBe(readFileSync(htmlFixture2Url, 'utf-8').replace(/\r/g, ''));
        expect(mail.text).toBe('\n');
        expect(mail.subject).toBe(msg.subject);
        expect(mail.from?.text).toBe(msg.from);
        expect(mail.to?.text).toBe(msg.to);
...
//# sourceMappingURL=message.integration.test.js.map