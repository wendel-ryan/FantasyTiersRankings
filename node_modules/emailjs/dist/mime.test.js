...
describe('mime', () => {
    ...
    it('mimeEncode should encode non UTF-8', () => {
        expect(mimeEncode(new Uint8Array([0xbd, 0xc5]), 'utf-16be')).toBe('=EB=B7=85');
    });
    it('mimeWordEncode should encode', () => {
        expect(mimeWordEncode('See on õhin test')).toBe('=?UTF-8?Q?See_on_=C3=B5hin_test?=');
    });
    it('mimeWordEncode should QP-encode mime word', () => {
        expect(mimeWordEncode(new Uint8Array([0x4a, 0xf5, 0x67, 0x65, 0x2d, 0x76, 0x61, 0xde]), 'Q', 'utf-16be')).toBe('=?UTF-8?Q?=E4=AB=B5=E6=9D=A5=E2=B5=B6=E6=87=9E?=');
    });
    it('mimeWordEncode should Base64-encode mime word', () => {
        expect(mimeWordEncode('Привет и до свидания', 'B')).toBe('=?UTF-8?B?0J/RgNC40LLQtdGCINC4INC00L4g0YHQstC40LTQsNC90LjRjw==?=');
    });
    ...
    it('splitMimeEncodedString should split long encoded strings', () => {
        const input = 'This is a very long encoded string that needs to be split into multiple parts because it exceeds the maximum length allowed for mime words in email headers';
