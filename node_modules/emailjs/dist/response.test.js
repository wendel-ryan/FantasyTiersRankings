...
describe('SMTPResponseMonitor', () => {
    ...
    let mockOnError;
    const TIMEOUT = 1000;
    let timeoutCallback; // Store the callback passed to setTimeout
    beforeEach(() => {
    ...
    it('should parse and emit a valid SMTP response', () => {
        const monitor = new SMTPResponseMonitor(mockStream, TIMEOUT, mockOnError);
    ...
    it('should emit error on stream "error" event', () => {
        ...
        expect(emittedError.message).toBe('connection encountered an error (Socket error)');
    ...
    it('should cover timeout handling', () => {
        ...
        const emittedError = responseSpy.mock.calls[0][0];
        expect(emittedError).toBeInstanceOf(SMTPError);
        expect(emittedError.message).toBe('timedout while connecting to smtp server');
